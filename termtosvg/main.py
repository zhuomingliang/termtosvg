...
def parse(args, templates, default_template, default_geometry, default_min_dur, default_max_dur,
    ...
    parser.add_argument(
        'output_path',
        nargs='?',
        help='optional filename of the SVG animation. If --still-frame is specified, '
             'output_path should be the path of the directory where still frames will be '
    ...
    if args:
        if args[0] == 'record':
            ...
            parser.add_argument(
                'output_path',
                nargs='?',
                help='optional filename of the cast file. If missing, a random path will be '
